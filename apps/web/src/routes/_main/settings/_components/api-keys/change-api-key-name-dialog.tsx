...
interface ChangeApiKeyNameDialogProps {
  handle: DialogPrimitive.Handle<{ apiKey: ApiKey }>;
}

export function ChangeApiKeyNameDialog({ handle }: ChangeApiKeyNameDialogProps) {
  const trpc = useTRPC();
  ...
  useFormDeps(form, [payload?.apiKey.id], {
    defaultValues: { displayName: payload?.apiKey.displayName ?? "" },
  });
  ...
  const onSubmit = form.handleSubmit((values) => {
    ...
    updateMutation.mutate({ id: payload.apiKey.id, displayName: values.displayName });
  });
      ...
      {!!payload && (
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>Rename {payload.apiKey.displayName}.</DialogDescription>
          </DialogHeader>

          <form onSubmit={onSubmit} noValidate className="flex flex-col gap-4">
            <Field>
              <FieldLabel htmlFor="change-api-key-display-name">Name</FieldLabel>
              <Input
                id="change-api-key-display-name"
                autoFocus
                autoComplete="off"
